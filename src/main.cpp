...
#if defined(NDEBUG)
# error "Jatcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb6ec3bfadaabca337157067b8a2802a509c8a7547e40054b7809eca9cab98a81");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Jatcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Jatcoin Signed Message:\n";
...
{
    // Jatcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Jatcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Jatcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Jatcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Jatcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jatcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf8ca4ef29185fba15576cb5637d1ea8c747cd592ba7098841464bd0c0118840f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043f56d31bae813c74704e5a11523ae6d9a2989935fe724b21d9a90b478f5bd23b3168ebd59b986cb270b65e5bbaba332309ae5b29715ea78033c1b8112c1444a1")<<OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1577278865;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084718989;

        if (fTestNet)
        {
            block.nTime    = 1577278563;
            block.nNonce   = 385448043;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0eaaf6573fcb745e269904494dcce292741c2e0a62069bef491be37048d7596e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc1, 0xb4, 0xdf }; // Jatcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JatcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JatcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JatcoinMiner(CWallet *pwallet)
{
    printf("JatcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jatcoin-miner");
    ...
    try { loop {
        ...
        printf("Running JatcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JatcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JatcoinMiner, pwallet));
}
